...
import { LayoutDashboard, ReceiptText, LogOut, Menu, User } from 'lucide-react';
  ...
  return (
    <header className='sticky top-0 z-40 w-full border-b border-border/60 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 lg:hidden'>
      <div className='flex h-14 items-center justify-between px-4'>
        ...
        >
          <span className='bg-gradient-to-r from-indigo-500 to-violet-500 bg-clip-text text-lg font-bold text-transparent'>
            {APP_NAME}
          </span>
        </Link>
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent
            side='right'
            className='w-[280px] border-l border-border/60 bg-background/95 backdrop-blur-xl p-0'
          >
            <motion.div
              initial='initial'
              animate='animate'
...
export function DesktopSidebar({ userEmail }: MobileSidebarProps) {
  return (
    <aside className='hidden lg:flex lg:w-[260px] lg:flex-col lg:fixed lg:inset-y-0 lg:left-0 lg:z-30 lg:border-r lg:border-border/60 lg:bg-card/50 lg:backdrop-blur-xl'>
